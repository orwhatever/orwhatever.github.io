...
window.onload = function init()
{
	...
	document.getElementById("ambient").onchange =
	function() {
		var val = event.srcElement.value;
		materialAmbient = vec4(val,val,val,1.0);
		var Product = mult(lightEmission, materialAmbient);
		gl.uniform4fv( gl.getUniformLocation(program,
			"ambientProduct"),flatten(Product) );
	}
	
	document.getElementById("diffuse").onchange =
	function() {
		var val = event.srcElement.value;
		materialDiffuse = vec4(val,val,val,1.0);
		var Product = mult(lightEmission, materialDiffuse);
		gl.uniform4fv( gl.getUniformLocation(program,
			"diffuseProduct"),flatten(Product) );
	}
	
	document.getElementById("specular").onchange =
	function() {
		var val = event.srcElement.value;
		materialSpecular = vec4(val,val,val,1.0);
		var Product = mult(lightEmission, materialSpecular);
		gl.uniform4fv( gl.getUniformLocation(program,
			"specularProduct"),flatten(Product) );
	}
	
	document.getElementById("light").onchange =
	function() {
		var val = event.srcElement.value;
		lightEmission = vec4(val,val,val,1.0);
		var ambientProduct = mult(lightEmission, materialAmbient);
		var diffuseProduct = mult(lightEmission, materialDiffuse);
		var specularProduct = mult(lightEmission, materialSpecular);
		gl.uniform4fv( gl.getUniformLocation(program,
